...
def search(request):
    if request.method == "POST":
        ...
        comments = Comment.objects.filter(Q(content__contains=searched))
        try:
            int(searched)
            blogs = BlogPost.objects.filter(Q(title__contains=searched)
                                            | Q(category__contains=searched)
                                            | Q(pk__in=comments.values_list('blog', flat=True))
                                            | Q(dateTime__year=searched)
                                            | Q(dateTime__month=searched)
                                            | Q(dateTime__day=searched))
        except ValueError:
            blogs = BlogPost.objects.filter(Q(title__contains=searched)
                                            | Q(category__contains=searched)
                                            | Q(pk__in=comments.values_list('blog', flat=True)))     
